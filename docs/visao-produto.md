...
- **Em aberto pra próxima etapa:** REQUESTS (pedidos de corrida/entrega/etc, fora de Grupos) continua sem noção de dono — diferente de perfil e grupo, REQUESTS também nasce via chat do site e via WhatsApp, sem sessão de navegador pra amarrar de forma confiável nesses dois casos; precisa de mais desenho antes de encarar (talvez só amarrar quando a criação vier do formulário direto, deixando chat/WhatsApp sem dono mesmo).
