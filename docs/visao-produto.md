...
- **Status (2026-09-13): implementado (login básico), com "editar perfil" e "painel pessoal" ainda em aberto.** Botão "Entrar com Google" (Google Identity Services) no menu, só aparece com `GOOGLE_CLIENT_ID` configurada. `google-auth-library` verifica o token no servidor (`/api/auth/google`), sessão em cookie assinado (`/api/auth/me`, `/api/auth/logout`). Perfil criado enquanto logado já grava `ownerUserId` — a base pra editar/painel já existe, só falta a interface pra usar isso.
