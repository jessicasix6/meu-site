...
- **Status (2026-09-14): painel pessoal ganhou "Meus grupos".** `ownerUserId` (mesmo padrão de `PROVIDER_PROFILES`) adicionado a `GROUP_OPPORTUNITIES` na criação — grupo continua podendo ser criado sem login, só fica sem dono nesse caso. `GET /api/auth/me` devolve `groups` (id, título, categoria, status) junto de `providers`; clicar em "Ver" no painel filtra a seção Grupos pra categoria certa. `ownerUserId` nunca aparece em nenhuma resposta pública (`GET /api/groups`, detalhe) — só usado internamente pro filtro do painel.
- **Status (2026-09-14): painel pessoal ganhou "Meus pedidos" também — fechando de vez a lista de "próxima etapa".** Igual imaginado: só o pedido publicado pelo formulário direto (`POST /api/requests`) grava `ownerUserId`; publicado por conversa (chat do site ou WhatsApp, ambos passam por `runPublishRequestTool`/`askAgent`, sem sessão de navegador) continua sem dono. `GET /api/auth/me` devolve `requests` junto de `providers`/`groups`. `ownerUserId` nunca aparece em nenhuma resposta pública de pedido (criar, listar, aceitar, concluir, avaliar) — todas passam por `requestSummary()`, mesmo padrão de `groupSummary()`. "Ver" no painel leva pro quadro geral (modo "Presto serviço") — não existe filtro por pedido específico ainda, mas o pedido da pessoa aparece lá junto dos outros.
- **Painel pessoal (pilar 4.13) está completo agora**, com as três seções: Meus perfis, Meus grupos, Meus pedidos.
