...
| `UMAMI_SCRIPT_URL` / `UMAMI_WEBSITE_ID` | não | URL do script de rastreamento e id do site numa instância do [Umami](https://umami.is/) self-hosted (task-008) — estatísticas de visita sem mandar dado pra terceiro (tipo Google Analytics). Sem as duas, o site funciona normal, só sem rastreamento nenhum. Diferente do MinIO, essa URL precisa ser alcançável de verdade pelo navegador de quem visita o site (o script roda no cliente) — não pode ser um endereço só interno. |
| `ALTCHA_HMAC_KEY` | não | Chave secreta (qualquer string aleatória longa) que assina os desafios do [ALTCHA](https://altcha.org/) (task-008) — proof-of-work anti-spam no cadastro e na criação de post em Grupos/Carona, sem CAPTCHA visual e sem serviço de terceiro. Sem ela, os formulários funcionam normalmente, sem nenhuma verificação anti-spam. |
